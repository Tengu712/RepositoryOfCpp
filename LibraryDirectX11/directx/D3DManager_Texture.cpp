...
bool D3DManager::createTexture(unsigned int id, Texture* pTexture) {
    try {
        ...
        if (FAILED(pFactory->CreateDecoderFromFilename(
                L"../res/hogetex.png", nullptr, GENERIC_READ, WICDecodeMetadataCacheOnDemand, pDecoder.GetAddressOf())))
        ...
        if (FAILED(pFrame->GetPixelFormat(&format)))
            throw "Failed to get pixel format.";
